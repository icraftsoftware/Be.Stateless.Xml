...
namespace Be.Stateless.Xml
{
	...
	public class XmlBuilderReader : XmlReader
	{
		...
		/// </returns>
		/// <remarks>
		/// This property is relevant to <see cref="XmlNodeType.DocumentType"/>, <see cref="XmlNodeType.Element"/>, and <see
		/// cref="XmlNodeType.XmlDeclaration"/> nodes only. Other node types do not have attributes.
		/// </remarks>
		public override int AttributeCount => CurrentNode.GetAttributes().Count();
		...
		/// limited to DTDs. The <see cref="BaseURI"/> tells you where these nodes came from. If there is no base URI for the
		/// nodes being returned (for example, they were parsed from an in-memory string), <c>String.Empty</c> is returned.
		/// </remarks>
		...
		/// </returns>
		public override int Depth => _enumerators.Count > 0 ? _enumerators.Count - 1 : 0;
		...
		/// </summary>
		/// <returns>
		/// The value of the specified attribute. If the attribute is not found or the value is <c>String.Empty</c>, <c>null</c>
		/// is returned.
		/// </returns>
		/// <param name="name">
		...
		/// </returns>
		public override bool HasAttributes => CurrentNode.HasAttributes();
		...
		/// </item>
		/// <item>
		/// <![CDATA[<item num="123"></item>]]>, for which <see cref="IsEmptyElement"/> is <c>false</c> although element content
		/// is empty.
		/// </item>
		/// </list>
		...
		/// </para>
		/// <para>
		/// If default content has been added to an element due to schema validation, <see cref="IsEmptyElement"/> still returns
		/// <c>true</c>. It has no bearing on whether or not the element has a default value. In other words, <see
		/// cref="IsEmptyElement"/> simply reports whether or not the element in the source document had an end element tag.
		/// </para>
		/// </remarks>
		public override bool IsEmptyElement => !CurrentNode.HasChildNodes();

		/// <summary>
		/// Gets the local name of the current node.
		/// </summary>
		/// <returns>
		/// The name of the current node with the prefix removed. For example, <see cref="LocalName"/> is <c>book</c> for the
		/// element &lt;bk:book&gt;. For node types that do not have a name (like <see cref="XmlNodeType.Text"/>, <see
		...
		/// <summary>
		/// Moves to the attribute with the specified <see cref="XmlReader.LocalName"/> and <see cref="XmlReader.NamespaceURI"/>.
		/// </summary>
		/// <returns>
		/// <c>true</c> if the attribute is found; <c>false</c> otherwise. If <c>false</c>, the reader's position does not
		/// change.
		...
		/// </summary>
		/// <returns>
		/// <c>true</c> if the reader is positioned on an attribute (the reader moves to the element that owns the attribute);
		/// <c>false</c> if the reader is not positioned on an attribute (the position of the reader does not change).
		/// </returns>
		/// <remarks>
		...
		/// </returns>
		/// <remarks>
		/// If <see cref="MoveToFirstAttribute"/> returns <c>true</c>, the reader moves to the first attribute; otherwise, the
		...
		/// </returns>
		/// <remarks>
		/// If the current node is an element node, this method is equivalent to <see cref="MoveToFirstAttribute"/>. If <see
		...
		/// </returns>
		/// <remarks>
		/// This property is relevant to <see cref="XmlNodeType.Attribute"/> and <see cref="XmlNodeType.Element"/> nodes only.
		/// </remarks>
		...
		/// </returns>
		/// <remarks>
		/// All node and attribute names returned from XmlReader are atomized using the <see cref="XmlNameTable"/>. When the same
		/// name is returned multiple times (for example, <c>Customer</c>), then the same <see cref="string"/> object will be
		...
		/// </remarks>
		public override XmlNameTable NameTable => _table;
		...
		/// </returns>
		public override string Prefix => CurrentNode.Prefix ?? string.Empty;
		...
		/// </summary>
		/// <returns>
		/// <c>true</c> if there are nodes to return. <c>false</c> if the reader is not positioned on an attribute node when the
		...
		/// </returns>
		/// <remarks>
		/// Use this method after calling <see cref="MoveToAttribute(string)"/> to read through the text or entity reference
		/// nodes that make up the attribute value. The <see cref="XmlReader.Depth"/> of the attribute value nodes is one plus
		...
		/// The reader is not positioned on an <see cref="XmlNodeType.EntityReference"/> node or this implementation of the
		/// reader cannot resolve entities (<see cref="XmlReader.CanResolveEntity"/> returns <c>false</c>).
		/// </exception>
		/// <remarks>
		/// If the reader is positioned on an <see cref="XmlNodeType.EntityReference"/> node, if <see cref="Read"/> is called
		/// after calling this method, the entity replacement text is parsed. When the entity replacement text is finished, an
		/// <see cref="XmlNodeType.EndEntity"/> node is returned to close the entity reference scope.
		/// </remarks>
		...
		/// </summary>
		/// <returns>
		/// The value returned depends on the <see cref="NodeType"/> of the node. The following table lists node types that have
		/// a value to return. All other node types return <c>String.Empty</c>.
		/// <list type="table">
		/// <listheader>
		...
		/// </list>
		/// </returns>
		public override string Value => CurrentNode.Value ?? string.Empty;
